...
`R markdown`: [package](https://rmarkdown.rstudio.com/) used to analyze and share all documents on www.github.com(http://github.com/)

`Tidyverse`: [package](https://tidyverse.tidyverse.org/) used to organize some data.
...
```{r summary_forest_plots, echo=FALSE, fig.height=6, fig.width=12, message=FALSE, warning=FALSE}

x <- rbind(
  c( "subgroup", "studies", "n", "N", "i2", "est", "llci", "ulci", "p"),
  extract_info_maglmm_proportions("Overall", inc2),
  c("Sex", rep(NA, 8)),
  extract_info_maglmm_proportions("Male", inc4, subgroup = T, sub_analysis = inc_sex_ft),
  extract_info_maglmm_proportions("Female", inc6),
  c("Age", rep(NA, 8)),
  ...
  extract_info_maglmm_proportions(">10mm", inc12, subgroup = T, sub_analysis = inc_tumorsize_ft),
  extract_info_maglmm_proportions("<10mm", inc14),
  c("USA-based", rep(NA, 8)),
  extract_info_maglmm_proportions("USA", inc16, subgroup = T, sub_analysis = inc_uscountry_ft),
  extract_info_maglmm_proportions("non-USA", inc18),
  c("Population-based", rep(NA, 8)),
  extract_info_maglmm_proportions("Population", inc20, subgroup = T, sub_analysis = inc_popbased_ft),
  extract_info_maglmm_proportions("non-Population", inc22)
  )
...
x <- x %>% add_row(.before = 2)
x <- x %>% add_row(.before = 6)
x <- x %>% add_row(.before = 10)
x <- x %>% add_row(.before = 14)
x <- x %>% add_row(.before = 18)
x <- x %>% add_row(.before = 1)
x <- x %>% add_row(.after  = 23)
...
sub1 <- c(6,7,10,11, 14,15, 18,19, 22,23) 
